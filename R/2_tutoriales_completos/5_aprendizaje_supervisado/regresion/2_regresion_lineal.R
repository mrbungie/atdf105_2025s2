...
# ## 5) Variables para regresión lineal (numéricas + estandarización)
# La regresión lineal busca una relación aproximadamente lineal entre predictores y precio.
# Sirve como línea base porque es fácil de interpretar.
# Modelo de regresión lineal
train_modelo <- bind_cols(y = y_train, x_train)
test_modelo <- bind_cols(y = y_test, x_test)
modelo <- lm(y ~ ., data = train_modelo)
...
graficar_resultados(y_test, pred_test, "Regresión lineal")
