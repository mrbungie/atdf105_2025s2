...
# ## 5) Variables para red neuronal (numéricas + estandarización)
# La red neuronal aprende relaciones no lineales combinando varias transformaciones internas.
# Puede capturar patrones complejos, pero suele ser menos interpretable.
# Red neuronal para regresión
train_modelo <- bind_cols(y = y_train, x_train)
test_modelo <- bind_cols(y = y_test, x_test)
modelo <- nnet(y ~ ., data = train_modelo, size = 5, linout = TRUE, maxit = 500, decay = 0.01, trace = FALSE)
print(modelo)

pred_train <- predict(modelo, newdata = train_modelo)
pred_test <- predict(modelo, newdata = test_modelo)
...
graficar_resultados(y_test, pred_test, "Red neuronal")
