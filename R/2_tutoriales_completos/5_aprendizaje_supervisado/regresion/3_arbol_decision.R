...
# ## 5) Variables para árbol de decisión (numéricas)
# El árbol construye reglas del tipo "si pasa esto, entonces predigo aquello".
# Es útil para captar relaciones no lineales de manera interpretable.
# Árbol de decisión para regresión
train_modelo <- bind_cols(y = y_train, x_train)
test_modelo <- bind_cols(y = y_test, x_test)
modelo <- rpart(y ~ ., data = train_modelo, method = "anova", control = rpart.control(maxdepth = 5))
...
graficar_resultados(y_test, pred_test, "Árbol de decisión")
