...
# SVR busca un equilibrio entre margen amplio y error controlado.
# También depende de distancias, por eso el escalado importa.
# Support Vector Regression
train_modelo <- bind_cols(y = y_train, x_train)
test_modelo <- bind_cols(y = y_test, x_test)
modelo <- svm(y ~ ., data = train_modelo, type = "eps-regression", kernel = "radial", cost = 10, epsilon = 0.1)
print(modelo)
...
graficar_resultados(y_test, pred_test, "SVM")
