# install.packages(c("tidyverse", "caret", "pROC", "nnet"))
library(nnet)
...
# ## 5) Variables para red neuronal (numericas + codificacion simple)
# Este modelo aprende relaciones no lineales con capas ocultas.
# Hiperparametros a ajustar: size, decay, maxit.
modelo <- nnet(
  x = as.matrix(x_train),
  y = class.ind(y_train_factor),
  size = 5,
  maxit = 300,
  decay = 0.1,
  softmax = TRUE,
...
pred_train_raw <- predict(modelo, as.matrix(x_train), type = "raw")
y_train_pred <- extraer_probabilidad_positiva(pred_train_raw)
y_train_pred_class <- as.integer(y_train_pred >= umbral)

pred_test_raw <- predict(modelo, as.matrix(x_test), type = "raw")
y_test_pred <- extraer_probabilidad_positiva(pred_test_raw)
