# install.packages(c("tidyverse", "caret", "pROC", "e1071"))
library(e1071)
...
# ## 5) Variables para Naive Bayes (numericas + codificacion simple)
# Este modelo asume independencia condicional entre predictores.
# Hiperparametros a ajustar: suavizado laplaciano.
train_nb <- bind_cols(Survived = y_train_factor, x_train)
test_nb <- bind_cols(Survived = y_test_factor, x_test)
modelo <- naiveBayes(Survived ~ ., data = train_nb)
print(modelo)

y_train_prob_raw <- predict(modelo, newdata = train_nb, type = "raw")
y_train_pred <- extraer_probabilidad_positiva(y_train_prob_raw)
y_train_pred_class <- as.integer(y_train_pred >= umbral)

y_test_prob_raw <- predict(modelo, newdata = test_nb, type = "raw")
y_test_pred <- extraer_probabilidad_positiva(y_test_prob_raw)
