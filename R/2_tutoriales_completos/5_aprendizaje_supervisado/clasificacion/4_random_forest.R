...
# ## 5) Variables para random forest (numericas + codificacion simple)
# Este modelo combina muchos árboles para mejorar estabilidad.
# Hiperparametros a ajustar: ntree, mtry, nodesize.
train_rf <- bind_cols(Survived = y_train_factor, x_train)
test_rf <- bind_cols(Survived = y_test_factor, x_test)
modelo <- randomForest(Survived ~ ., data = train_rf, ntree = 200)
print(modelo)
print(importance(modelo))

y_train_prob <- predict(modelo, newdata = train_rf, type = "prob")
y_train_pred <- extraer_probabilidad_positiva(y_train_prob)
y_train_pred_class <- as.integer(y_train_pred >= umbral)

y_test_prob <- predict(modelo, newdata = test_rf, type = "prob")
