...
# ## 5) Variables para SVM (numericas + codificacion simple)
# Este modelo busca un hiperplano que separe las clases.
# Hiperparametros a ajustar: cost, gamma, kernel.
train_svm <- bind_cols(Survived = y_train_factor, x_train)
test_svm <- bind_cols(Survived = y_test_factor, x_test)
modelo <- svm(Survived ~ ., data = train_svm, kernel = "radial", probability = TRUE)
print(modelo)

pred_train_factor <- predict(modelo, newdata = train_svm, probability = TRUE)
y_train_prob <- extraer_probabilidad_positiva(attr(pred_train_factor, "probabilities"))
y_train_pred <- y_train_prob
y_train_pred_class <- as.integer(as.character(pred_train_factor))

pred_test_factor <- predict(modelo, newdata = test_svm, probability = TRUE)
y_test_prob <- extraer_probabilidad_positiva(attr(pred_test_factor, "probabilities"))
