...
# ## 5) Variables para regresión logística (numericas + codificacion simple)
# Este modelo estima la probabilidad de supervivencia con un enlace logit.
# Hiperparametros a revisar: interacciones, regularización, selección de variables.
train_glm <- bind_cols(Survived = y_train_num, x_train)
test_glm <- bind_cols(Survived = y_test_num, x_test)
modelo <- glm(Survived ~ ., data = train_glm, family = binomial())
print(summary(modelo))
coeficientes <- tibble(variable = names(coef(modelo)), coeficiente = as.numeric(coef(modelo))) %>%
  arrange(desc(abs(coeficiente)))
print(coeficientes)

y_train_pred <- predict(modelo, newdata = train_glm, type = "response")
y_train_pred_class <- as.integer(y_train_pred >= umbral)

y_test_pred <- predict(modelo, newdata = test_glm, type = "response")
