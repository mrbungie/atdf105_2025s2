# install.packages(c("tidyverse", "caret", "pROC", "rpart"))
library(rpart)
...
# ## 5) Variables para árbol de decisión (numericas + codificacion simple)
# Este modelo divide el espacio de variables en reglas simples.
# Hiperparametros a ajustar: maxdepth, minsplit, cp.
train_tree <- bind_cols(Survived = y_train_factor, x_train)
test_tree <- bind_cols(Survived = y_test_factor, x_test)
modelo <- rpart(Survived ~ ., data = train_tree, method = "class", control = rpart.control(maxdepth = 5))
print(summary(modelo))
plot(modelo)
text(modelo, use.n = TRUE, cex = 0.7)

y_train_prob <- predict(modelo, newdata = train_tree, type = "prob")
y_train_pred <- extraer_probabilidad_positiva(y_train_prob)
y_train_pred_class <- as.integer(y_train_pred >= umbral)

y_test_prob <- predict(modelo, newdata = test_tree, type = "prob")
y_test_pred <- extraer_probabilidad_positiva(y_test_prob)
y_test_pred_class <- as.integer(y_test_pred >= umbral)
