# install.packages(c("tidyverse", "caret", "bnlearn"))
library(bnlearn)
...
# ## 5) Variables para red bayesiana (discretizacion + inferencia probabilistica)
# Este modelo requiere variables discretas y aprende una estructura probabilística.
# Hiperparametros a ajustar: discretización y estructura.

discretizar_bn <- function(train_df, test_df) {
  fare_breaks <- unique(quantile(train_df$Fare, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE))
  if (length(fare_breaks) < 2) {
    fare_breaks <- c(min(train_df$Fare, na.rm = TRUE), max(train_df$Fare, na.rm = TRUE))
  }
  if (length(fare_breaks) == 2 && fare_breaks[1] == fare_breaks[2]) {
    fare_breaks <- c(fare_breaks[1] - 1, fare_breaks[2] + 1)
  }

  preparar_bn <- function(df) {
    df %>%
      mutate(
        Survived = factor(Survived, levels = c(0, 1)),
        Pclass = factor(Pclass),
        Age_disc = cut(Age, breaks = c(-Inf, 18, 35, 60, Inf), labels = c("Joven", "Adulto", "Maduro", "Mayor")),
        Fare_disc = cut(Fare, breaks = fare_breaks, include.lowest = TRUE),
        SibSp_disc = case_when(SibSp == 0 ~ "Ninguno", SibSp <= 2 ~ "Pocos", TRUE ~ "Muchos"),
        Parch_disc = case_when(Parch == 0 ~ "Ninguno", Parch <= 2 ~ "Pocos", TRUE ~ "Muchos")
      ) %>%
      select(Survived, Pclass, Sex, Age_disc, SibSp_disc, Parch_disc, Fare_disc, Embarked) %>%
      mutate(across(everything(), as.factor))
  }

  list(train_bn = preparar_bn(train_df), test_bn = preparar_bn(test_df))
}

bn_data <- discretizar_bn(entrenamiento, prueba)
entrenamiento_bn <- bn_data$train_bn
prueba_bn <- bn_data$test_bn

estructura <- hc(entrenamiento_bn)
modelo <- bn.fit(estructura, entrenamiento_bn)
print(modelo)

pred_train_factor <- predict(modelo, node = "Survived", data = entrenamiento_bn, method = "bayes-lw")
y_train_pred <- NULL
y_train_pred_class <- as.integer(as.character(pred_train_factor))

pred_test_factor <- predict(modelo, node = "Survived", data = prueba_bn, method = "bayes-lw")
y_test_pred <- NULL
...
")
metricas_train <- evaluar_clasificacion(y_train_num, y_train_pred_class, NULL, "Entrenamiento")

cat("
=== Matriz de confusion prueba ===
")
metricas_test <- evaluar_clasificacion(y_test_num, y_test_pred_class, NULL, "Prueba")
