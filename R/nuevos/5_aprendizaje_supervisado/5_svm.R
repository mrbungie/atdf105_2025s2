...
# install.packages(c("tidyverse", "Metrics", "caret", "e1071"))
...
# Construir modelo SVM
modelo <- svm(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked,
              data = entrenamiento,
              probability = TRUE)
...
predicciones_train <- predict(modelo, newdata = entrenamiento, probability = TRUE)
predicciones_prob_train <- attr(predicciones_train, "probabilities")
predicciones_clase_train <- predicciones_train
...
metricas_train <- entrenamiento %>%
  mutate(
    ...
    probabilidad = predicciones_prob_train[, "1"]
  )

# ===== PREDICCIONES Y MÉTRICAS PARA PRUEBA =====
# Hacer predicciones en el conjunto de prueba
predicciones_test <- predict(modelo, newdata = prueba, probability = TRUE)
predicciones_prob_test <- attr(predicciones_test, "probabilities")
predicciones_clase_test <- predicciones_test
...
metricas_test <- prueba %>%
  mutate(
    ...
    probabilidad = predicciones_prob_test[, "1"]
  )
