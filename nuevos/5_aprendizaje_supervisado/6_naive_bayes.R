...
# Construir modelo Naive Bayes
modelo <- naiveBayes(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked,
                     data = entrenamiento)
...
predicciones_train <- predict(modelo, newdata = entrenamiento, type = "raw")
...
predicciones_test <- predict(modelo, newdata = prueba, type = "raw")
